...
Here you could use other options like ``menuconfig`` or ``oldconfig`` instead of ``xconfig``.

Enable ``Rust support`` (``CONFIG_RUST``) in the ``General setup``
menu::

  General Setup
          -> Rust Support

The option is only shown if a suitable Rust toolchain is found (see
