...
use crate::{
    bindings, device, device_id, driver,
...
impl<T: Driver> driver::DriverOps for Adapter<T> {
    ...
    fn register(
        pdrv: &mut bindings::platform_driver,
        name: &'static CStr,
    ...
    fn unregister(pdrv: &mut bindings::platform_driver) {
...
impl<T: Driver> Adapter<T> {
    fn get_id_info(dev: &Device) -> Option<&'static T::IdInfo> {
        ...
        let id = unsafe { bindings::of_match_device(table.as_ref(), dev.as_ref().as_raw()) };
        if id.is_null() {
            return None;
        }
...
pub trait Driver {
    ...
    const OF_DEVICE_ID_TABLE: Option<device_id::IdTable<'static, of::DeviceId, Self::IdInfo>> =
        None;
