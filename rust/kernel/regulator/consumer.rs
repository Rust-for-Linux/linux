...
impl Regulator {
    ...
    /// created
    pub fn get<T: AsRef<Device>>(dev: T, id: &'static CStr) -> Result<Self> {
        // SAFETY: `dev.as_raw() is valid and non-null by the type invariant and
    ...
    pub fn get_optional<T: AsRef<Device>>(dev: T, id: &'static CStr) -> Result<Self> {
        // SAFETY: `dev.as_raw() is valid and non-null by the type invariant and
        // id has a static lifetime so it lives indefinitely
        let reg = from_err_ptr(unsafe {
    ...
    pub fn get_exclusive<T: AsRef<Device>>(dev: T, id: &'static CStr) -> Result<Self> {
        // SAFETY: `dev.as_raw() is valid and non-null by the type invariant and
        // id has a static lifetime so it lives indefinitely
        let reg = from_err_ptr(unsafe {
