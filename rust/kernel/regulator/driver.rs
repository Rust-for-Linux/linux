...
//!         let reg = Registration::register(pdev, &DESC, Config::<Self::Data>::new(pdev))?;
//!         Ok(())
//!     }
//! }
//!
...
// SAFETY: `Desc` cannot be modified after its declaration, hence it is safe to share references
// between threads.
...
/// # use kernel::regulator::driver::{Desc, Registration};
/// # use kernel::{device::Device, sync::Arc};
///
/// struct DriverData(u32);
///
/// # fn probe(dev: &Device, desc: &'static Desc) -> Result {
/// let config = Config::<Arc<DriverData>>::new(&dev, Arc::try_new(DriverData(128))?);
/// let reg = Registration::register(dev, desc, config)?;
/// #     Ok(())
/// # }
/// ```
///
...
impl<T: ForeignOwnable + Send + Sync> Config<T> {
    ...
    pub fn new<U: AsRef<Device>>(dev: U, data: T) -> Self {
        Self {
            cfg: bindings::regulator_config {
                dev: dev.as_ref().as_raw(),
                ..Default::default()
            },
            data,
            regmap: None,
...
impl Registration {
    ...
    pub fn register<T: ForeignOwnable + Send + Sync, U: AsRef<Device>>(
        dev: U,
        desc: &'static Desc,
        cfg: Config<T>,
    ) -> Result<Self> {
...
impl Regulator {
    /// register a Regulator driver
    fn register<T: ForeignOwnable + Send + Sync, U: AsRef<Device>>(
        dev: U,
        desc: &'static Desc,
        mut config: Config<T>,
    ) -> Result<Self> {
        ...
        let rdev = from_err_ptr(unsafe {
            bindings::regulator_register(dev.as_ref().as_raw(), &desc.0, &config.cfg)
        })?;
