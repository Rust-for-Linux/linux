...
use crate::{bindings, device_id, str::BStr};
...
impl DeviceId {
    #[doc(hidden)]
    pub const fn to_rawid(&self, offset: isize) -> <Self as device_id::RawDeviceId>::RawType {
        let DeviceId::Compatible(compatible) = self;
        let mut id = <Self as device_id::RawDeviceId>::ZERO;
        let mut i = 0;
        while i < compatible.len() {
            // If `compatible` does not fit in `id.compatible`, an "index out of bounds" build time
            // error will be triggered.
            id.compatible[i] = compatible.deref_const()[i] as _;
            i += 1;
        }
