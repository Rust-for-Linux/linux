...
pub fn rtnl_link_ops(ts: TokenStream) -> TokenStream {
    let mut it = ts.into_iter();
    let literals = vec!["maxtype", "policy", "slave_maxtype", "slave_policy"];
    ...
    format!(
        ...
        // TODO: add #[link_section = ".data.read_mosty"] if x86
        #[cfg(any(CONFIG_X86, CONFIG_SPARC64))]
        #[link_section = ".data.read_mostly"]
        {}

        #[cfg(not(any(CONFIG_X86, CONFIG_SPARC64)))]
        {}
        "#,
        callbacks, ops_struct, ops_struct,
    )
    .parse()
    .expect("Error parsing formatted string into token stream.")
}
...
impl RtnlLinkValues {
    fn new(callback_params: &str, wrapper_before: &str, params: &str) -> Self {
        Self {
            callback_params: callback_params.to_string(),
            return_type: "()".to_string(),
            wrapper_before: wrapper_before.to_string(),
            wrapper_after: "".to_string(),
...
fn get_rtnl_links_values(name: &str, netdevice: &str) -> RtnlLinkValues {
    ...
    match name {
        ...
        "validate" => RtnlLinkValues {
            callback_params: "tb: *mut *mut kernel::bindings::nlattr, data: *mut *mut kernel::bindings::nlattr, extack: *mut kernel::bindings::netlink_ext_ack".to_string(),
            return_type: "kernel::c_types::c_int".to_string(),
            wrapper_before: r#"kernel::from_kernel_result! {
                ...
                "#.to_string(),
            wrapper_after: "?; Ok(0) }".to_string(),
            params: "&tb, &data, &extack".to_string(),
        },
